...
/**
 * 
 * @author jean philippe cette classe expose les methodes d'interaction avec la
 *         base de donnée
 *
 */
@Repository
public class jdbcSuspectDAO implements SuspectDAO {
	...
	/**
	 * methode de verification pour eviter les doublons de creation au niveau de
	 * l'insert
	 */
	@Override
	...
	/**
	 * Methode qui recupère un objet Suspect par son id pour le passer en parametre
	 * de archiverSuspect
	 * 
	 * @param id
	 * @return Suspect
	...
	/**
	 * Methode d'archivage de suspect par transaction manuelle effectue insert dans
	 * la table archive_humain puis delete jointure ( personne_impliquee) et enfin
	 * delete suspect. Si une requete ne passe pas aucune requete n'est executée.
	 * 
	 * @param Suspect
	 */
	@Override
	public void archiverSuspect(Suspect suspect) {
		...
		try {
			...
			try {
				String sqlInsert = " INSERT INTO archive_personne_impliquee ( id_archive_personne_impliquee , nom, prenom, genre, date_naissance, nationalite, taille, poids, adresse, signe_distinctif,photo, empreinte, casier,nombre_condamnation, type_condamnation ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
				pstmtInsert = dbconnect.prepareStatement(sqlInsert);
				pstmtInsert.setInt(++i, suspect.getId());
				pstmtInsert.setString(++i, suspect.getNom());
				pstmtInsert.setString(++i, suspect.getPrenom());
				pstmtInsert.setString(++i, suspect.getGenre());
				pstmtInsert.setDate(++i, suspect.getDateNaissance());
				pstmtInsert.setString(++i, suspect.getNationalite());
				pstmtInsert.setFloat(++i, suspect.getTaille());
				pstmtInsert.setInt(++i, suspect.getPoids());
				pstmtInsert.setString(++i, suspect.getAdresseConnues());
				pstmtInsert.setString(++i, suspect.getSigneDistinctif());
				pstmtInsert.setString(++i, suspect.getPhoto());
				pstmtInsert.setString(++i, suspect.getEmpreinte());
				pstmtInsert.setBoolean(++i, suspect.isCasierJudiciaire());
				pstmtInsert.setInt(++i, suspect.getCondamnations());
				pstmtInsert.setString(++i, suspect.getTypeCondamnation());
			...
			try {
				...
				pstmtDeleteJointure.setInt(1, suspect.getId());
				// Run the the update query
			...
			try {
				...
				pstmtDelete.setInt(1, suspect.getId());
				// Run the the update query
	...
	/***
	 * Cette methode set la valeur de chaque attribut de la classe suspect en
	 * recuperant les informations de la base de donnée via le ResultSet
	 * 
	 * @param rs
	 * @return suspect
	 * @throws SQLException
...
/**
 * essai transaction manuelle conservée à titre d'exemple de mauvaise pratique requete multi select
 * */
	...
	//
	//
	// String insertSql = " INSERT INTO archive_personne_impliquee
	// (id_archive_personne_impliquee, nom , prenom , date_naissance , genre , photo
	// , date_deces , adresse , grade , competences , date_prise_service , actif ,
	// telephone , taille , poids , signe_distinctif , empreinte , casier ,
	// nombre_condamnation , type_condamnation , nationalite , statut ) VALUES
	// ((SELECT id_humain FROM humain WHERE id_humain = ?), (SELECT nom FROM humain
	// WHERE id_humain = ?), (SELECT prenom FROM humain WHERE id_humain = ?),
	// (SELECT date_naissance FROM humain WHERE id_humain = ?), (SELECT genre FROM
	// humain WHERE id_humain = ?), (SELECT photo FROM humain WHERE id_humain = ?),
	// (SELECT date_deces FROM humain WHERE id_humain = ?), (SELECT adresse FROM
	// humain WHERE id_humain = ?), (SELECT grade FROM humain WHERE id_humain = ?),
	// (SELECT competences FROM humain WHERE id_humain = ?), (SELECT
	// date_prise_service FROM humain WHERE id_humain = ?), (SELECT actif FROM
	// humain WHERE id_humain = ?), (SELECT telephone FROM humain WHERE id_humain =
	// ?), (SELECT taille FROM humain WHERE id_humain = ?), (SELECT poids FROM
	// humain WHERE id_humain = ?), (SELECT signe_distinctif FROM humain WHERE
	// id_humain = ?), (SELECT empreinte FROM humain WHERE id_humain = ?), (SELECT
	// casier FROM humain WHERE id_humain = ?), (SELECT nombre_condamnation FROM
	// humain WHERE id_humain = ?), (SELECT type_condamnation FROM humain WHERE
	// id_humain = ?), (SELECT nationalite FROM humain WHERE id_humain = ?), (SELECT
	// status_id FROM personne_impliquee WHERE humain_id = ?))" ;
	...
	//
	// try {
	// //String sql = " START TRANSACTION ; INSERT INTO archive_personne_impliquee
	// (id_archive_personne_impliquee, nom , prenom , date_naissance , genre , photo
	// , date_deces , adresse , grade , competences , date_prise_service , actif ,
	// telephone , taille , poids , signe_distinctif , empreinte , casier ,
	// nombre_condamnation , type_condamnation , nationalite , statut ) VALUES
	...
	// humain WHERE id_humain = ?), (SELECT type_condamnation FROM humain WHERE
	// id_humain = ?), (SELECT nationalite FROM humain WHERE id_humain = ?), (SELECT
	// status_id FROM personne_impliquee WHERE humain_id = ?)) ; DELETE FROM
	// personne_impliquee WHERE humain_id = ? ; DELETE FROM humain WHERE id_humain =
	// ?; COMMIT ; ";
	// dbConnection.setAutoCommit(false);
	//
	// pstmtInsert = datasource.getConnection().prepareStatement(insertSql,
	// PreparedStatement.RETURN_GENERATED_KEYS);
	// pstmtDelete1 = datasource.getConnection().prepareStatement(deleteSql1,
	// PreparedStatement.RETURN_GENERATED_KEYS);
	// pstmtDelete2 = datasource.getConnection().prepareStatement(deleteSql2,
	// PreparedStatement.RETURN_GENERATED_KEYS);
	//
	// for (int j=1 ; j<25 ; j++) {
	...
	// int i =0;
	// try {
	// String sql = "INSERT INTO archive_personne_impliquee
	// (id_archive_personne_impliquee, nom , prenom , date_naissance , genre , photo
	// , date_deces , adresse , grade , competences , date_prise_service , actif ,
	// telephone , taille , poids , signe_distinctif , empreinte , casier ,
	// nombre_condamnation , type_condamnation , nationalite , statut ) VALUES
	...
	//
	// pstmt =
	// datasource.getConnection().prepareStatement(sql,PreparedStatement.RETURN_GENERATED_KEYS);
	// for (int j=1 ; j<23 ; j++) {
	// pstmt.setInt(++i, id);
	// }
	//
	...
	// try {
	// String insertSql = " INSERT INTO archive_personne_impliquee (
	// id_archive_personne_impliquee , nom, prenom, genre, date_naissance,
	// nationalite, taille, poids, adresse, signe_distinctif,photo, empreinte,
	// casier,nombre_condamnation, type_condamnation ) VALUES
	// (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	// pstmt2 = datasource.getConnection().prepareStatement(insertSql,
	// PreparedStatement.RETURN_GENERATED_KEYS);
